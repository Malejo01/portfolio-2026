...
export default function NotFound() {
  const copy = errors["en"];
  const { nav, cases } = getContent("en");

  return (
    <StatusPage label={copy.notFoundLabel} title={copy.notFoundTitle} body={copy.notFoundBody}>
      <ButtonRow>
        <ButtonLink href={localized("/", "en")} tone="solid" surface="paper">
          {nav.backToHome}
        </ButtonLink>
        <ButtonLink href={localized("/#casos", "en")} surface="paper">
          {cases.label}
        </ButtonLink>
      </ButtonRow>
    </StatusPage>
